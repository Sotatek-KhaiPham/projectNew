...
import { getResultPayment } from '../../../microservices';
...
export class OrdersService {
  ...
  async updateStatusOrderWithCode(
    ...
    try {
      ...
      if (order?.code === code && order?.status === OrderStatus.CREATED) {
        ...
        const resultPayment = await getResultPayment(valuesPayments);
  ...
  async cancelOrder(
    ...
    try {
      ...
      const order = await this.order.findOne({ id: id, email });
      if (
